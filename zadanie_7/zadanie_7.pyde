...
class Bee(Pet):
    def __init__(self, name):
        self.name = name
    def speak(self):
        text('bzzzt', random(50, width-70), random(50, height-50))
        return 'bzzzt'
    def __sub__(self, other):
        return self.name + other.name + other.name[::-1] + self.name[::-1]
...
def setup():
    ...
    barryBenson = Bee('Barry B Benson') # moja ulubiona pszczola
    adam = Bee('Adam') # a to jego przyjaciel
    #janusz = Bunny() # odkomentujcie i sprawdźcie błąd, usuńcie dziedziczenie po Pet z klasy i sprawdźcie ponownie
    global list_of_pets
    list_of_pets = [rex, benio, skrypcik, barryBenson, adam] # lista zawiera różne typy zwierząt
    ...
    print(barryBenson - adam)
    print(adam - barryBenson)
